...
public class TransactionRecordDaoImpl implements TransactionRecordDao
{
    ...
    {
        String sql = "select id, user_id, create_time, type, amount, target_user_id, newBalance from transaction_record where user_id = ?";
                ...
                PreparedStatement ps = conn.prepareStatement(sql))
        {
            ...
            try (ResultSet rs = ps.executeQuery())
            {
                while (rs.next())
                {
                    ...
                    transactionRecord.setUserId(rs.getInt("user_id"));
